...
export default function LeaguesPage() {
  ...
  const [lobbyCredentials, setLobbyCredentials] = useState<Record<number, { roomId: string; roomPassword: string }>>({});
  ...
  // Start editing a lobby's credentials
  ...
  const handleSaveCredentials = async (lobbyId: number) => {
    ...
    if (!creds?.roomId || !creds?.roomPassword) {
      setMessage({ type: "error", text: "Please fill in both Room ID and Password" });
      return;
    }
    ...
    try {
      const res = await secureFetch(`/api/admin/leagues/${lobbiesModal.tournament.id}/lobbies`, {
        ...
        body: JSON.stringify({
          ...
          lobbyId,
          roomId: creds.roomId,
          roomPassword: creds.roomPassword,
        }),
      });
      ...
      if (data.success) {
        ...
        // Refresh lobbies
        const lobbiesRes = await secureFetch(`/api/admin/leagues/${lobbiesModal.tournament.id}/lobbies`);
        const lobbiesData = await lobbiesRes.json();
        if (lobbiesData.success) {
          setLobbiesModal(prev => ({ ...prev, lobbies: lobbiesData.data.lobbies }));
        }
      } else {
  ...
  const handlePublishCredentials = async (lobbyId: number) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setMessage({ type: "success", text: `Credentials published! Notified ${data.data.notifiedCount || 0} team members.` });
        // Refresh lobbies
        const lobbiesRes = await secureFetch(`/api/admin/leagues/${lobbiesModal.tournament.id}/lobbies`);
        const lobbiesData = await lobbiesRes.json();
        if (lobbiesData.success) {
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        >
          <div className="space-y-4">
            ...
            <div className="hidden">NEW_CODE_LOADED_2026</div>
